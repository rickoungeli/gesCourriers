<div class="titre2">Les courriers orient&eacute;s vers DIRCAB et PR</div>
...
$nbreannonce = $courrier->TotalMessageSynthese();
$AnnoncesParPage = 40;
...
{
echo '<center><span class="titre3">R&eacute;sultats de la recherche</span> | <a href="index.php?module=dircab&action=synthese" class="message">Afficher Tout</a></center><br>';
}
...
$req = "select * from courrier where expediteur like '%".$_POST['expediteur']."%' and destinataire like '%".$_POST['destinataire']."%' and objet like '%".$_POST['objet']."%'  $type_courrier $dated and reception > 0 AND ((orientation = 6 and categorie = 1) OR (orientation = '8' and categorie = 1) OR tuteur =  '6' OR tuteur = '8') order by id DESC";
}
else
{
$req = "SELECT * FROM courrier WHERE reception > 0 AND ((orientation = 6 and categorie = 1) OR (orientation = '8' and categorie = 1) OR tuteur =  '6' OR tuteur = '8') ORDER BY id DESC LIMIT $PremiereAnnonceAafficher , $AnnoncesParPage ";
}
//////echo $req;
$courrier->affichersynthese($req);
echo "<br>";
...
if($groupe > 1) {
$group = $groupe - 1;
echo '<div align="center"><a class="message" href="index.php?module=dircab&action=synthese&page=' . $debut . '&groupe='.$group.'">&lt;&lt;&lt;&nbsp;Pr&eacute;c&eacute;dent&nbsp;&nbsp;&nbsp;&nbsp;</a>';
}

echo '<b>Page&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></span>';
for ($i = $debut ; $i <= $fin ; $i++)
{
echo '<a class="message" href="index.php?module=dircab&action=synthese&page=' . $i . '&groupe='.$groupe.'">' . $i . '</a> '; 
echo "&nbsp;&nbsp;&nbsp;";
}
$totalgroup = ceil($NbreDePages/20);
$group = $groupe + 1;
if($group <= $totalgroup ) {
echo '<a class="message" href="index.php?module=dircab&action=synthese&page=' . $i . '&groupe='.$group.'">Suivant&nbsp;&gt;&gt;&gt;</a>';
}}
?>
</form>
