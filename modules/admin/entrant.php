...
<form id="form1" action="modules/reception/supprimer.php" method="post">
    <?php
require_once("classes/courrier.class.inc.php");
$courrier = new courrier;

$nbreannonce = $courrier->TotalMessageSaisi();
$AnnoncesParPage = 40;
...
{
echo '<center><span class="titre3">R&eacute;sultats de la recherche</span> | <a href="index.php?module=admin&action=entrant" class="message">Afficher Tout</a></center><br>';
}
...
$req = "select * from courrier where categorie = 1 and expediteur like '%".addslashes($_POST['expediteur'])."%' and destinataire like '%".addslashes($_POST['destinataire'])."%' and objet like '%".addslashes($_POST['objet'])."%'  $type_courrier $dated order by id DESC";
}
else
{
$req = "SELECT * FROM courrier WHERE categorie='1' ORDER BY id DESC LIMIT $PremiereAnnonceAafficher , $AnnoncesParPage ";
}
//echo $req;
$courrier->afficheradmin($req);
echo "<br>";
...
if($groupe > 1) {
$group = $groupe - 1;
echo '<div align="center"><a class="message" href="index.php?module=admin&action=entrant&page=' . $debut . '&groupe='.$group.'">&lt;&lt;&lt;&nbsp;Pr&eacute;c&eacute;dent&nbsp;&nbsp;&nbsp;&nbsp;</a>';
}

echo '<b>Page&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></span>';
for ($i = $debut ; $i <= $fin ; $i++)
{
echo '<a class="message" href="index.php?module=admin&action=entrant&page=' . $i . '&groupe='.$groupe.'">' . $i . '</a> '; 
echo "&nbsp;&nbsp;&nbsp;";
}
$totalgroup = ceil($NbreDePages/20);
$group = $groupe + 1;
if($group <= $totalgroup ) {
echo '<a class="message" href="index.php?module=admin&action=entrant&page=' . $i . '&groupe='.$group.'">Suivant&nbsp;&gt;&gt;&gt;</a>';
}}
?>
  <p align="center">Pour la s&eacute;lection  : 
    <label>
    <input type="submit" name="sendtri" id="sendtri" value="Envoyer pour tri" />
    </label>
    <label>
